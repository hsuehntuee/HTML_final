...
X_val = validation_df[required_columns].fillna(validation_df[required_columns].median()).to_numpy().astype(float)[1301:1500]
y_true = validation_df['home_team_win'].to_numpy()[1301:1500]
X_val_poly = poly.transform(X_val)
X_val_scaled = scaler.transform(X_val_poly)

# 取得各模型的預測機率
probas = np.array([clf.predict_proba(X_val_scaled)[:, 1] for clf in voting_model.named_estimators_.values()])
avg_proba = (probas[3])  # 計算平均機率
