...
# 測試集處理
newo = pd.read_csv('filled_kaggle_test2.csv')
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, Y)):
    ...
    # 建立 AdaBoost 模型
    base_learner = DecisionTreeClassifier(max_depth=1)  # 基礎小階次元件
    model = AdaBoostClassifier(estimator=base_learner, n_estimators=500, learning_rate=1.0, random_state=1)
...
print("Test predictions saved to 'result_adaboost.csv'.")
