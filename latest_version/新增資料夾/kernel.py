...
param_grid = {
    'C': [0.01, 0,1, 1, 10, 100],  # Regularization parameter, testing a wider range
    'degree': [1],     # Degree of the polynomial kernel, usually 2 to 4 is a good range
    'coef0': [0, 0.5, 1, 5, 10],  # Constant term, typically in the range of 0 to 10
}
### 0.01,2, 10
...
predictions2 = best_svm_model.predict(X_test)
result_df = pd.DataFrame({'id': np.arange(len(predictions2)), 'home_team_win': predictions2.flatten()})
result_df.to_csv('result_poly.csv', index=False)
print("Test predictions saved to 'result_polykernel.csv'.")